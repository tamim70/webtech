...
<body>
	...
	<script>
		document.getElementById("login_form").addEventListener("submit", validate)

		function validate(event) {
			event.preventDefault()
			const uname = document.getElementsByName("u_name")[0].value
			const password = document.getElementsByName("u_pass")[0].value
			if (password.length < 6)
				return alert("Password too short")
			const req = new XMLHttpRequest();
			req.open("POST", "./login.php");
			req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
			req.send(`u_name=${uname}&u_pass=${password}`);
		}
	</script>
