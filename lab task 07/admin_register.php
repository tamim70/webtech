...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (isset($_POST['reg'])) {
		...
		if (empty($_POST['fullname'])) {
			$NameError = "Fullname cannot be empty!";
			$flag = 1;
		}
		...
		if (empty($_POST['hiredate'])) {
			$DateError = "Hiredate cannot be empty!";
			$flag = 1;
		}
		...
		if ($flag == 0) {
			...
			if (!validate_username($username)) {
				$UserError = "Username Not Available(Minimum 4 Character)!";
				$flag = 1;
			}
			...
			if (!validate_hiredate($hiredate)) {
				...
				$DateError = "Hiredate Can Not be In Future";
				$flag = 1;
			}

			if (!validate_phone($contact)) {

				$ContactError = "Invalid Contact Information";
				$flag = 1;
			} else if (!validate_phone_existence($contact)) {
				$ContactError = "Phone Number Already Used";
				$flag = 1;
			}
			...
			if (!validate_email($mail)) {
				...
				$MailError = "Email Already used";
				$flag = 1;
			}


			if (!validate_password($pass)) {
				$PassError = "Minimum Lenght of Password is 4";
				$flag = 1;
			}
				...
				if (file_exists('data.json')) {
					...
					if (file_put_contents('data.json', $final_data)) {
						$_SESSION['userid'] = $extra["id"];
						$_SESSION['username'] = $extra["username"];
...
<body>
	...
	<div class="regTop">
		...
		<p class="txt">ADMIN REGISTRATION</p>
	</div>
