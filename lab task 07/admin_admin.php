...
$sql_table = "select *  from admin where id != ".$_SESSION['userid']." and username !='admin' ORDER BY hiredate DESC ;";
...
if($_SERVER["REQUEST_METHOD"]=="POST")
  {
    ...
    else
    {
      ...
      if (isset($_POST['Resign'])) {
            ...
            $date = date('Y-m-d');
            $sql = "UPDATE `admin` SET `resignDate` = '".$date."' WHERE `id` = ".$_POST['Resign'].";";
            mysqli_query($conn,$sql);
            header("Refresh:0");
        }
        elseif (isset($_POST['Recall'])) {
            $sql = "UPDATE `admin` SET `resignDate` = NULL WHERE `id` = ".$_POST['Recall'].";";
...
<body>
	...
	<section id="board">
		...
		<a href="admin_register.php"> <button class="add_btn" >Add new Admin</button></a>
